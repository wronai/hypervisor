...
def _deterministic_weather_plan(prompt: str) -> dict[str, Any]:
    ...
    return {
        "domain": {"id": domain_id, "name": "weather-map", "description": "Generate forecast weather maps as HTML URL artifacts."},
...
def _generic_plan(prompt: str) -> dict[str, Any]:
    ...
    return {
        "domain": {"id": domain_id, "name": slug, "description": prompt},
        "standards": {"uri_tree_schema": "schemas/uri_tree.schema.json", "data_contract": "protobuf", "agent_manifest": "a2a_agent_card", "resource_model": "mcp_resources"},
        ...
        "artifacts": {},
        "agent": {"id": agent_id, "name": agent_id, "card_uri": f"a2a://{agent_id}/.well-known/agent-card.json", "capabilities": [{"name": "run", "type": "command", "uri": f"a2a://{agent_id}/skills/run"}]},
...
def _call_openrouter(prompt: str) -> dict[str, Any]:
    ...
    with httpx.Client(timeout=60) as client:
        r = client.post(f"{base_url}/chat/completions", headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}, json=payload)
        r.raise_for_status()
        content = r.json()["choices"][0]["message"]["content"]
    return _extract_json(content)


def plan_domain_from_prompt(prompt: str, use_llm: bool | None = None) -> dict[str, Any]:
    if use_llm is None:
