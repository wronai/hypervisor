...
def test_example_03_ssh_docker_testenv(repo_root: Path, examples_env):
    ...
    try:
        env = {**examples_env, "HYPERVISOR_SSH_PASSWORD": examples_env.get("HYPERVISOR_SSH_PASSWORD", "deploy")}
        scan = run_shell(repo_root, ["uri3", "scan", "ssh"], env=env, timeout_s=120)
        assert scan.returncode == 0, scan.stderr
    finally:
        down = run_shell(repo_root, ["make", "docker-testenv-down"], env=examples_env, timeout_s=120)
...
def test_example_15_playwright_mock_via_uri3(repo_root: Path, examples_env):
    ...
    validate = run_shell(repo_root, ["uri3", "validate-workflow", str(graph)], env=examples_env)
    assert validate.returncode == 0
    run = run_shell(
        repo_root,
        ["uri3", "run-workflow", str(graph), "--approve", "--browser", "mock"],
        env=examples_env,
        timeout_s=120,
    )
