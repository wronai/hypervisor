...
def _process_actions(agent_id: str, related: dict[str, str]) -> list[UriAction]:
    return [
...
def _process_view_context(
    ...
    *,
    ...
    return deployment, inspection, readiness, summary
...
def build_process_view(agent_id: str, *, root: Path | None = None) -> ProcessViewModel:
    deployment, inspection, readiness, summary = _process_view_context(agent_id, root=root)
